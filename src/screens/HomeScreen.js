...
export default function HomeScreen(props) {
          ...
          if (userToken == "xyz" || userToken == null) {
            ...
            if (showSearch == false || showSearch == null) {
              ...
              const getData = async () => {
                ...
                setQuote(QuoteData.data.posts.reverse());
                const PoemData = await axios.get(`${BACKEND}/filter/POEM`);
                setPoem(PoemData.data.posts.reverse());
                const ShayariData = await axios.get(
                  `${BACKEND}/filter/SHAYARI`
                );
                setShayari(ShayariData.data.posts.reverse());
                const StoryData = await axios.get(`${BACKEND}/filter/STORY`);
                setStory(StoryData.data.posts.reverse());
                const ArtData = await axios.get(`${BACKEND}/filter/ART`);
                setArt(ArtData.data.posts.reverse());
  ...
  useEffect(() => {
    (async () => {
      const getNoti = await axios.get(`${BACKEND}/updateYesorNo`);
